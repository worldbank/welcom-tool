...
#delimit;
capture program drop prjob51;
program define prjob51, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname) HGroup(varname) LAN(string) XRNAMES(string)  AGGRegate(string) PCEXP(varname)  WAPPR(int 1) MODEL(int 1) GVIMP(int 0) SUBS(real 0.4)  PLINE(string) MATPEL(string) MATIEL(string) SOTM(int 1)];
...
prjobpov `tot_imp',    hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(total)  pcexp(`pcexp') alpha(0)  pline(`pline')  ;
tempname mat51tot ;
...
prjobpov `tot_imp',    hs(`hsize') aehs(`aehs')  lan(`lan')    xrnames(`slist')   pcexp(`pcexp') alpha(0)  pline(`pline')  ;
tempname mat51tot ;
matrix `mat51tot'= e(est); 
};
...
prjobpov `flist',    hs(`hsize') aehs(`aehs')  lan(`lan')    xrnames(`slist')   pcexp(`pcexp') alpha(0)  pline(`pline') ;
