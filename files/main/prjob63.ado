...
#delimit;
capture program drop prjob63;
program define prjob63, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname) HGroup(varname) LAN(string) XRNAMES(string)  AGGRegate(string) PCEXP(varname)  WAPPR(int 1) MODEL(int 1) GVIMP(int 0) SUBS(real 0.4)  THETA(real 0) STEP(int 1) MATPEL(string) MATIEL(string) SOTM(int 1)];
...
mcjobentropy `tot_imp',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')      theta(`theta') ;
tempname mat63tot ;
...
cap drop  __imwmc;
prjobentropy `tot_imp',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')      theta(`theta') ;
tempname mat63tot ;
matrix `mat63tot'= e(est); 
};
...
prjobentropy `flist',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')      theta(`theta') ;
