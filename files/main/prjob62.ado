...
#delimit;
capture program drop prjob62;
program define prjob62, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname) HGroup(varname) LAN(string) XRNAMES(string)  AGGRegate(string) PCEXP(varname)  WAPPR(int 1) MODEL(int 1) GVIMP(int 0) SUBS(real 0.4)  EPSILON(real 0.5) STEP(int 1) MATPEL(string) MATIEL(string) SOTM(int 1)];
...
mcjobatkinson `tot_imp',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')      epsilon(`epsilon') ;
tempname mat62tot ;
...
prjobatkinson `tot_imp',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')      epsilon(`epsilon') ;
tempname mat62tot ;
matrix `mat62tot'= e(est); 
};
...
if (`wappr'==2)    imwmc_el ``i'' , prc(_prc`i')  elas(_elas`i') hsize(`hsize') ;
cap drop  __imwmc;
mcjobatkinson `tot_imp',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')      epsilon(`epsilon') ;
tempname mat62tot ;
matrix `mat62tot'= e(est); 
...
prjobatkinson `flist',   hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(`slist')  pcexp(`pcexp')      epsilon(`epsilon') ;
