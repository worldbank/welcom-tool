...
# delimit ;
cap program drop xml_taba2;
program define xml_taba2;
version 8;

syntax [anything] [, STYle(string) EXcelpath(passthru) CAlcpath(passthru) DMAT(int 1) updateopts Noisily CEQ1(string) *];
...
if (`dmat' == 1) {;
	if ("`ceq1'"~="") {;
			...
			local title : subinstr local title ">" "&gt;", all;
			local rc1 = `rc' - 1;
			if "`ceq1'" ~= "" local lceq1 "<Cell ss:StyleID='s`fno'_Title'  ss:MergeAcross='`rc1'' ss:HRef="#table_11!A1"> <Data ss:Type='String'>`ceq1'</Data></Cell>" ;
			local ltitle "<Row ss:Index='`addindex++'' ss:AutoFitHeight='1' ss:Height='22'> >
			              <Cell ss:StyleID='s`fno'_Title'   ss:HRef="#table_11!A1">
