...
#delimit;
capture program drop prjob52;
program define prjob52, eclass;
version 9.2;
syntax varlist(min=1)[, HSize(varname) HGroup(varname) LAN(string) XRNAMES(string)  AGGRegate(string) PCEXP(varname)  WAPPR(int 1) MODEL(int 1) GVIMP(int 0) SUBS(real 0.4) PLINE(string) STEP(int 1) MATPEL(string) MATIEL(string) SOTM(int 1)];
...
prjobpov `tot_imp',    hs(`hsize') aehs(`aehs')  lan(`lan')   xrnames(total)  pcexp(`pcexp') alpha(1)  pline(`pline')  ;
tempname mat52tot ;
...
cap drop  __imwmc;
prjobpov `tot_imp',    hs(`hsize') aehs(`aehs')  lan(`lan')    xrnames(`slist')   pcexp(`pcexp') alpha(1)  pline(`pline')  ;
tempname mat52tot ;
matrix `mat52tot'= e(est); 
};
...
prjobpov `flist',    hs(`hsize') aehs(`aehs')  lan(`lan')    xrnames(`slist')   pcexp(`pcexp') alpha(1)  pline(`pline')  ;
